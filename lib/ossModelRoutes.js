...
export const FULL_MODEL_REGISTRY = {
  ...
  qwen25_72b_instruct: {
    ...
    bestFor: ["hoshino"],            // 星野：情绪陪伴首选（豆包级中文自然度）
    defaultTemp: 0.85,
    tier: "free",                     // 免费版可用
  },
  qwen3_235b_a3b: {
    ...
    bestFor: ["mingchuan"],           // 明川老师：复杂家族系统深度思考
    defaultTemp: 0.7,
    tier: "growth",
  },
  deepseek_v3: {
    ...
    bestFor: ["kawa"],                // 川：代码导师首选（接近 Claude 3.5 代码）
    defaultTemp: 0.55,
    tier: "free",
  },
  deepseek_r1: {
  ...
  doubao_pro_32k: {
    ...
    bestFor: ["hoshino"],                // 星野 → 豆包 Pro，就是真正的豆包陪伴感
    defaultTemp: 0.92,
    tier: "growth",
    tags: ["best-companion"],
  },
  claude_35_sonnet: {
    ...
    bestFor: ["kawa"],                    // 川 → Claude 3.5，就是真正的顶级代码体验
    defaultTemp: 0.5,
    tier: "family",                       // 家庭版解锁
    tags: ["best-code"],
  },
  ...
  auto: {
    ...
    sfModelId: null,
    bestFor: ["hoshino","kawa","mingchuan"],
    defaultTemp: 0.7,
    tier: "free",
    tags: ["auto"],
  },
...
export function resolveFullModel({ charId = "hoshino", userTier = "free", preferredModelId = null, contentHint = null } = {}) {
  // 手动指定优先，但如果用户版本不够就回落
...
export function getModelListForUI(userTier = "free") {
  ...
  groups.forEach(g => g.items.forEach(it => {
    ...
    it._tierLabel = it.tier === "free" ? "🆓 免费" : it.tier === "growth" ? "🌱 成长版" : it.tier === "family" ? "👨‍👩‍👧 家庭版" : "🏫 校园版";
  }));
  return groups;
}
