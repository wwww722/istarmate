...
// ===== 作品展示墙 =====
export async function publishSnippet(userId, snippetId, authorDisplay) {
  await sql`
    UPDATE code_snippets
    SET is_public = TRUE, author_display = ${authorDisplay}
...
export async function getPublicSnippets(limit = 30) {
  const rows = await sql`
    SELECT id, title, language, author_display, created_at, LEFT(code, 300) as preview
    FROM code_snippets
...
export async function getFullPublicSnippet(id) {
  const rows = await sql`
    SELECT id, title, language, author_display, code, created_at
    FROM code_snippets
    WHERE id = ${id} AND is_public = TRUE LIMIT 1
  `;
  return rows[0] || null;
}

// ===== 危机检测 =====
export async function saveCrisisLog(userId, messageSnippet, riskLevel, context) {
  await sql`
    INSERT INTO crisis_logs (user_id, message_snippet, risk_level, context)
    VALUES (${userId}, ${messageSnippet}, ${riskLevel}, ${context})
  `;
}

export async function getRecentCrisisLogs(userId, hours = 24) {
  const rows = await sql`
...
export async function getOrCreateInviteCode(userId) {
  const existing = await sql`SELECT code FROM invite_codes WHERE user_id = ${userId} LIMIT 1`;
  if (existing[0]) return existing[0].code;
  const code = "ISM" + Math.random().toString(36).slice(2, 8).toUpperCase();
...
export async function getInviteCodeInfo(code) {
  const rows = await sql`
    SELECT ic.*, u.id as owner_id FROM invite_codes ic
...
export async function recordInviteUse(code, inviterId, inviteeId) {
  await sql`
    INSERT INTO invite_uses (invite_code, inviter_id, invitee_id)
    VALUES (${code}, ${inviterId}, ${inviteeId})
    ON CONFLICT DO NOTHING
  `;
  await sql`UPDATE invite_codes SET used_count = used_count + 1 WHERE code = ${code}`;
}

export async function getInviteStats(userId) {
  const rows = await sql`
    SELECT used_count FROM invite_codes WHERE user_id = ${userId} LIMIT 1
  `;
  return { inviteCount: rows[0]?.used_count || 0 };
}
