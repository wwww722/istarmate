...
// 惰性初始化：构建时可能没有 DATABASE_URL，延迟到真正查询时才连接，
// 避免 next build 阶段因缺环境变量而报错。
let _sql = null;
function getSql() {
  if (!_sql) {
    _sql = neon(process.env.DATABASE_URL || "postgresql://placeholder:placeholder@localhost/placeholder");
  }
  return _sql;
}
...
// 供函数式调用：neon 支持 sql(string, params) 直接调用（不是 .query 方法）
const sqlClient = { query: (text, params) => getSql()(text, params) };
...
// ===== 兼容函数式 sql 调用（Trae 升级版 chat.js 需要）=====
// 用法：sqlQuery("SELECT ... WHERE x=$1", [val]) → 返回 { rows: [...] }
...
export async function publishSnippet(userId, snippetId, authorDisplay) {
  await sql`
    UPDATE code_snippets
    SET is_public = TRUE, author_display = ${authorDisplay || '匿名作者'}
    WHERE id = ${snippetId} AND user_id = ${userId}
  `;
}
...
export async function getFullPublicSnippet(id) {
  const rows = await sql`
    SELECT id, title, code, language, description, author_display, created_at
    FROM code_snippets WHERE id = ${id} AND is_public = TRUE LIMIT 1
  `;
  return rows[0] || null;
}

// 用户自定义称呼
export async function setCalledAs(userId, calledAs) {
  await sql`UPDATE profiles SET called_as = ${calledAs || ''} WHERE user_id = ${userId}`;
}
