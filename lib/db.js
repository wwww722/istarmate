...
// ===== 每月成长报告 =====
export async function saveMonthlyReport(userId, yearMonth, report) {
  await sql`
    INSERT INTO monthly_reports (user_id, year_month, report)
    VALUES (${userId}, ${yearMonth}, ${JSON.stringify(report)})
    ON CONFLICT (user_id, year_month) DO UPDATE SET report = ${JSON.stringify(report)}
  `;
}

export async function getMonthlyReports(userId, limit = 6) {
  const rows = await sql`
    SELECT * FROM monthly_reports WHERE user_id = ${userId}
...
export async function getScenarioStoryMemories(userId, limit = 5) {
  const rows = await sql`
    SELECT summary, session_date FROM chat_summaries
...
export async function saveMonthlyReport(userId, yearMonth, report) {
  await sql`
    INSERT INTO weekly_reports (user_id, week_start, report)
    VALUES (${userId}, ${yearMonth + '-01'}, ${JSON.stringify({ ...report, type: 'monthly' })})
    ON CONFLICT (user_id, week_start) DO UPDATE SET report = ${JSON.stringify({ ...report, type: 'monthly' })}
  `;
}
