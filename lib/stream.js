...
export async function streamSiliconFlow(res, systemPrompt, messages, maxTokens = 600, model = null) {
  ...
  try {
    r = await fetch("https://api.siliconflow.cn/v1/chat/completions", {
      ...
      body: JSON.stringify({
        // 消息里含图片时自动切换到视觉模型
        model: model || (hasImage(messages) ? "zai-org/GLM-4.5V" : "Pro/zai-org/GLM-5.1"),
