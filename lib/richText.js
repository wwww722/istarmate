...
const LANGUAGES = {
  javascript: ["js", "javascript", "jsx", "ts", "typescript", "tsx"],
  html: ["html", "htm"],
  css: ["css", "scss", "sass"],
  python: ["python", "py"],
  bash: ["bash", "shell", "sh"],
...
function getLangCategory(lang) {
  ...
  for (const [category, aliases] of Object.entries(LANGUAGES)) {
...
function highlightCode(code, lang) {
  ...
  const category = getLangCategory(lang);

  if (category === "javascript" || category === "html") {
    html = html.replace(/(["'`])(?:(?=(\\?))\2.)*?\1/g, '<span style="color:#a6e3a1">$&</span>');
    html = html.replace(/(\/\/[^\n]*)/g, '<span style="color:#6c7086">$1</span>');
    html = html.replace(/\b(const|let|var|function|return|if|else|for|while|import|export|from|class|extends|new|await|async|try|catch|finally|throw|switch|case|default|break|continue|return|yield|typeof|instanceof|in|of|delete|void|debugger|with)\b/g, '<span style="color:#cba6f7">$1</span>');
    html = html.replace(/\b(true|false|null|undefined|NaN|Infinity|this|super)\b/g, '<span style="color:#fab387">$1</span>');
    ...
    html = html.replace(/([a-zA-Z][a-zA-Z0-9]*)\s*(?=\()/g, '<span style="color:#89b4fa">$1</span>');
  }

  if (category === "css") {
    html = html.replace(/(["'`])(?:(?=(\\?))\2.)*?\1/g, '<span style="color:#a6e3a1">$&</span>');
    html = html.replace(/(\/\*[\s\S]*?\*\/)/g, '<span style="color:#6c7086">$1</span>');
    html = html.replace(/\b(px|em|rem|vh|vw|%|s|ms)\b/g, '<span style="color:#fab387">$1</span>');
    html = html.replace(/\b(\d+\.?\d*)\b/g, '<span style="color:#fab387">$1</span>');
    html = html.replace(/\b(selectors|property|value)\b/g, '<span style="color:#89b4fa">$1</span>');
    html = html.replace(/([a-zA-Z-][a-zA-Z0-9-]*)\s*(?=:)/g, '<span style="color:#cba6f7">$1</span>');
  }

  if (category === "python") {
    html = html.replace(/(["'`])(?:(?=(\\?))\2.)*?\1/g, '<span style="color:#a6e3a1">$&</span>');
    html = html.replace(/(#.*)/g, '<span style="color:#6c7086">$1</span>');
    html = html.replace(/\b(def|class|return|if|elif|else|for|while|import|from|as|try|except|finally|raise|with|lambda|yield|async|await|pass|break|continue|del|global|nonlocal|print|True|False|None|and|or|not|in|is)\b/g, '<span style="color:#cba6f7">$1</span>');
    html = html.replace(/\b(\d+\.?\d*)\b/g, '<span style="color:#fab387">$1</span>');
  }
  ...
  if (category === "bash") {
    html = html.replace(/(["'`])(?:(?=(\\?))\2.)*?\1/g, '<span style="color:#a6e3a1">$&</span>');
    html = html.replace(/(#.*)/g, '<span style="color:#6c7086">$1</span>');
    html = html.replace(/\$([a-zA-Z_][a-zA-Z0-9_]*)/g, '<span style="color:#fab387">$$1</span>');
    html = html.replace(/\b(cd|ls|mkdir|rm|cp|mv|echo|cat|grep|sed|awk|chmod|chown|tar|zip|unzip|npm|yarn|git|docker|kubectl|curl|wget|python|node|java)\b/g, '<span style="color:#89b4fa">$1</span>');
  }

  if (category === "json") {
    html = html.replace(/\b(true|false|null)\b/g, '<span style="color:#fab387">$1</span>');
    html = html.replace(/\b(\d+\.?\d*)\b/g, '<span style="color:#fab387">$1</span>');
    html = html.replace(/"([^"]+)":/g, '<span style="color:#cba6f7">"$1"</span>:');
