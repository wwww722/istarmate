...
import {
  ...
  upsertScenarioProgress, getRecentChatSummaries
} from "../../lib/db";
import { todayDateStr } from "../../lib/scenarios";

async function generateTodayScenario(questionnaire, dateStr, userId) {
  ...
  let storyContext = "";
  try {
    const memories = await getRecentChatSummaries(userId, 4);
    const scenarioMemories = memories.filter(m => m.summary && m.summary.startsWith("[剧场]"));
    if (scenarioMemories.length > 0) {
      storyContext = `\n【过去几天的故事摘要，今天可以有延续性也可以是全新的故事】\n${scenarioMemories.map(m => "- " + m.summary.replace("[剧场] ", "")).join("\n")}`;
    }
  } catch {}
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (!scenario) {
      ...
      scenario = await generateTodayScenario(questionnaire, dateStr, userId);
    }
