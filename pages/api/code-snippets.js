...
import {
  saveCodeSnippet, getCodeSnippets, getCodeSnippet, deleteCodeSnippet,
  getPublicSnippets, toggleSnippetPublic, unlockAchievement, getAchievements,
  saveCodeVersion, updateCodeSnippet,
} from "../../lib/db";
...
export default async function handler(req, res) {
  ...
  if (req.method === "POST") {
    const { title, code, language, snippetId, versionNote } = req.body || {};

    // 带 snippetId：给已有项目保存新版本（迭代）
    if (snippetId) {
      if (!code) return res.status(400).json({ error: "缺少代码" });
      await updateCodeSnippet(userId, Number(snippetId), code);
      await saveCodeVersion(userId, Number(snippetId), code, versionNote || "");
      return res.status(200).json({ ok: true, snippetId: Number(snippetId) });
    }
    ...
    if (snippet?.id) {
      await saveCodeVersion(userId, snippet.id, code, "初始版本");
    }
